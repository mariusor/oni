...
_image_name=${1:-oni:"${_environment}"}
...
echo "Building image ${_image_name} for host=${_hostname} env:${_environment} version:${_version} port:${_listen_port}"
...
buildah run \
    ...
    make ENV="${_environment}" VERSION="${_version}" FEDBOX_HOSTNAME="${_hostname}" clean all cert
